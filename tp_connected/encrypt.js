  ...
  ($.rsa = $.rsa || {}),
    ($.rsa.encrypt = function(val, nn, ee) {
      ...
      if (256 != result.length)
        for (var l = Math.abs(256 - result.length), i = 0; i < l; i++)
          result = "0" + result;
      return result;
    }),
        ...
        2 == padding
          ...
          : 1 == padding
            ? encrypt &&
              ...
              8 === temp && (len += 8))
            : padding || (message += "\0\0\0\0\0\0\0\0");
      var result = "",
        tempresult = "";
      for (
        1 == mode &&
        ...
        for (
            ...
            1 == mode &&
              (encrypt
                ...
                : ((cbcleft2 = cbcleft),
                  ...
                  (cbcleft = left),
                  (cbcright = right))),
            left ^= (temp = 252645135 & ((left >>> 4) ^ right)) << 4,
            left ^= (temp = 65535 & ((left >>> 16) ^ (right ^= temp))) << 16,
            left ^= temp = 858993459 & (((right ^= temp) >>> 2) ^ left),
            left ^= temp = 16711935 & (((right ^= temp << 2) >>> 8) ^ left),
            left =
              ((left ^=
                (temp = 1431655765 & ((left >>> 1) ^ (right ^= temp << 8))) <<
                1) <<
                1) |
              (left >>> 31),
          ...
          for (
            endloop = looping[j + 1], loopinc = looping[j + 2], i = looping[j];
            i != endloop;
            i += loopinc
          )
            (right1 = right ^ keys[i]),
              ...
              (right =
                ...
                (spfunction2[(right1 >>> 24) & 63] |
                  ...
                  spfunction8[63 & right1] |
                  spfunction1[(right2 >>> 24) & 63] |
                  spfunction3[(right2 >>> 16) & 63] |
                  spfunction5[(right2 >>> 8) & 63] |
                  spfunction7[63 & right2]));
          (temp = left), (left = right), (right = temp);
        }
        (right = (right >>> 1) | (right << 31)),
          (right ^= temp =
            1431655765 &
            (((left = (left >>> 1) | (left << 31)) >>> 1) ^ right)),
          (right ^=
            (temp = 16711935 & ((right >>> 8) ^ (left ^= temp << 1))) << 8),
          (right ^= (temp = 858993459 & ((right >>> 2) ^ (left ^= temp))) << 2),
          (right ^= temp = 65535 & (((left ^= temp) >>> 16) ^ right)),
          (right ^= temp = 252645135 & (((left ^= temp << 16) >>> 4) ^ right)),
          (left ^= temp << 4),
          1 == mode &&
            (encrypt
      ...
      for (
        ...
        var left =
          ...
          right =
            ...
            key.charCodeAt(m++);
        (left ^= (temp = 252645135 & ((left >>> 4) ^ right)) << 4),
          (left ^= temp = 65535 & (((right ^= temp) >>> -16) ^ left)),
          (left ^=
            (temp = 858993459 & ((left >>> 2) ^ (right ^= temp << -16))) << 2),
          (left ^= temp = 65535 & (((right ^= temp) >>> -16) ^ left)),
          (left ^=
            (temp = 1431655765 & ((left >>> 1) ^ (right ^= temp << -16))) << 1),
          (left ^= temp = 16711935 & (((right ^= temp) >>> 8) ^ left)),
          (temp =
            ((left ^=
              (temp = 1431655765 & ((left >>> 1) ^ (right ^= temp << 8))) <<
              1) <<
